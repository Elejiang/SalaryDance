...
职责：定义 AI 协作规则、红线、提交规范和 Spec 文档维护要求。

薪动是 SwiftUI + AppKit 混合实现的 macOS 菜单栏 App，核心入口是状态栏、弹窗和设置窗口。
...
只有用户明确要求提交（如“提交git”“提交git记录”），才进行 git 提交。提交信息格式使用 `<type>: <subject>`，例如 `feat: v1.0`。默认采用中文。
...
| 层级 | 文件 | 加载时机 | 内容 |
|------|------|----------|------|
| L1 | `SPEC.md` | 需要项目全貌、业务概念和模块关系时 | 项目总览 |
...
- Spec 文档是长期维护资产；长期契约变化必须同步更新。
- 每次任务收尾检查配置字段、核心计算规则、刷新机制、打包流程、模块边界和开发流程是否需要更新。
- 文档与代码不一致时，以代码为准，并主动更新文档。
- 一次性功能迭代、局部修复、实现细节微调不沉淀到 `SPEC.md`、L2 文档或项目规范文档，除非它改变长期约定或会影响后续开发判断。
- 针对某次需求的业务变化、局部 UI 调整或实现细节，如果没有形成长期规则、配置契约、核心计算口径、刷新机制或模块边界变化，不写入 `SPEC.md` 或 L2 文档。
