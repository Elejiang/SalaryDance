...
- 弹窗宽度固定 280。
- 当前收入、今日剩余、状态、进度、各薪资指标、摸鱼状态、提前下班与加班、打工语录均可单独开关。
- 弹窗提前下班与加班面板在当前可提前下班、可加班、当天已有提前下班/加班记录或加班进行中时展示；提前下班和加班都支持撤回。
- 弹窗提前下班与加班面板不展示金额指标网格，默认只展示操作、加班时分输入和一句今日提示文案；状态入口可手动开启但默认关闭。脱敏打开时提示中的金额需要脱敏。
- 加班时长控件需支持键盘输入和点击上下调整，分钟点击步幅为 5 分钟；0 时 0 分时不可开始加班，按钮悬浮提示原因。
- 加班进行中时展示预计加班和当前已加班两行文案，并提供提前终止入口；提前终止会以当前时刻结算本次加班。
- 弹窗今日摸鱼摘要有独立展示开关，默认开启；工作中、未开始、休息中和下班后都使用同一套当日摸鱼统计文案。
- 弹窗摸鱼状态内可单独开关本日、本周、本月/本周期、历史的摸鱼薪资和摸鱼时长；脱敏打开时摸鱼金额也需要脱敏，时长不脱敏。
- 真实弹窗和预览应优先共用同一 SwiftUI 组件；主薪资区复用 `SalaryDisplayView`，提前下班/加班和摸鱼状态区复用共享面板，避免按钮、脱敏小眼睛和展示布局出现两套实现。
...
- 记录页手动新增和编辑提前下班只输入实际下班时间，手动新增和编辑加班只输入加班日期和时长；同一工作日只能保存一条提前下班或加班记录，同日再次保存更长加班时长时延长已有加班记录；历史明细行提供单条编辑和删除入口，保存按钮需要随同新增一致的校验结果置灰。
