...
- 弹窗宽度固定 280。
- 当前收入、今日剩余、状态、进度、各薪资指标、摸鱼状态、提前下班与晚下班、打工语录均可单独开关。
- 弹窗提前下班与晚下班面板在当前可提前下班、可晚下班、当天已有提前下班/晚下班记录或晚下班进行中时展示；提前下班和晚下班都支持撤回。
- 弹窗提前下班与晚下班面板不展示金额指标网格，默认只展示操作、晚下班时分输入和一句今日提示文案；状态入口可手动开启但默认关闭。脱敏打开时提示中的金额需要脱敏。
- 晚下班时长控件需支持键盘输入和点击上下调整，分钟点击步幅为 5 分钟；0 时 0 分时不可开始晚下班，按钮悬浮提示原因。
- 晚下班进行中时展示预计晚下班和当前已晚下班两行文案，并提供提前终止入口；提前终止会以当前时刻结算本次晚下班。
- 弹窗今日摸鱼摘要有独立展示开关，默认开启；工作中、未开始、休息中和下班后都使用同一套当日摸鱼统计文案。
- 弹窗摸鱼状态内可单独开关本日、本周、本月/本周期、历史的摸鱼薪资和摸鱼时长；脱敏打开时摸鱼金额也需要脱敏，时长不脱敏。
- 真实弹窗和预览应优先共用同一 SwiftUI 组件；主薪资区复用 `SalaryDisplayView`，提前下班/晚下班和摸鱼状态区复用共享面板，避免按钮、脱敏小眼睛和展示布局出现两套实现。
...
- 薪资、时间、记录、应用等页面只负责输入、预览和操作入口；业务字段划分和计算规则以 `docs/config-and-calculation.md` 为准。
- 记录页摸鱼数据概览和历史记录中的摸鱼时长有“占总时长”百分比；数据概览分母使用本日/本周/本周期完整排班计薪时长，历史统计分母按首条记录日期和自然年/月规则计算到当前已发生计薪时长。
- 记录页在休息时间不计薪且历史摸鱼记录被休息段影响时，需要提示记录会保留，但完全落在不计薪休息段的记录不进入摸鱼薪资、时长和次数统计。
- 记录页手动新增和编辑提前下班只输入实际下班时间，手动新增和编辑晚下班只输入晚下班日期和时长；同一工作日只能保存一条提前下班或晚下班记录，同日再次保存更长晚下班时长时延长已有晚下班记录；历史明细行提供单条编辑和删除入口，保存按钮需要随同新增一致的校验结果置灰。
